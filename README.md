...
### Запись диалога в историю проекта

Переписка с ботом может складываться в `docs/history/<дата>.md` того проекта, который
сейчас активен: сообщения пользователя пишет мост, ответы — `tg-send.sh` и `tg-ask.sh`.
Вывод терминала туда не попадает — он через эти точки не проходит.

Нужно это публичным учебным репозиториям (`my_portal`), где переписка «что попросили →
что вышло» лежит рядом с кодом. **По умолчанию выключено везде.** Включается созданием
файла `docs/history/.enabled` в каталоге проекта; удаление файла запись прекращает.

Домашние пути, IP-адреса и всё похожее на токен заменяются заглушками (`history-log.sh`).
